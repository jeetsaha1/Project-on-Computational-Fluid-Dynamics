...
!**************************                                            *****************************
     ...
     $ AN,BN,DW,JN,CN,A,ff,si,T,T1
      ...
      real(kind=8), parameter :: BLANK = -1.0d30
...
!**********************************************************************!
      ...
      do 210 i = 1, nx
         do 220 j = 1, ny
            ...
            if ( x(i).lt.x_step .and. y(j).gt.(LY-h_step) ) then
               solid(i,j) = .true.
            endif
...
!.......................... Upper wall ................................!
      do  i = 1, nx
         ...
         if (x(i) .lt. x_step) then
            jwall = int((LY-h_step)/dyy) + 1
         else
            jwall = ny
         endif

         u(i,jwall)  = -u(i,jwall-1)
         u1(i,jwall) = -u1(i,jwall-1)
         ...
         cpp(i,jwall)  = cpp(i,jwall-1)
         cpp1(i,jwall) = cpp1(i,jwall-1)
...
!----------------------------------------------------------------------!
!---------- SOLID ENFORCEMENT (SAFE POSITION) --------------------------
...
!**********************************************************************!
     ...
     $          + (v1(i,j)-v1(i,j-1))*dx(i)
            ...
            if (.not. solid(i+1,j)) then
               rhs  = rhs  + cpp(i+1,j)/dxr(i)
               pcof = pcof + dy(j)/dxr(i)
            endif

            if (.not. solid(i-1,j)) then
               rhs  = rhs  + cpp1(i-1,j)/dxl(i)
               pcof = pcof + dy(j)/dxl(i)
            endif

            if (.not. solid(i,j+1)) then
               rhs  = rhs  + cpp(i,j+1)/dyt(j)
               pcof = pcof + dx(i)/dyt(j)
            endif

            if (.not. solid(i,j-1)) then
               rhs  = rhs  + cpp1(i,j-1)/dyb(j)
               pcof = pcof + dx(i)/dyb(j)
            endif

            rhs  = (2.0d0/3.0d0)*dt*rhs
            pcof = (2.0d0/3.0d0)*dt*pcof

            if (pcof .lt. 1.0d-12) then
               cpp1(i,j) = 0.0d0
            else
...
!--------------------------------------------------
! Remove pressure correction inside solid cells
!--------------------------------------------------
      do i = 1, nx
        do j = 1, ny
          if (solid(i,j)) then
...
!**********************************************************************!
      ...
      if((bigu.gt.Error).or.(bigv.gt.Error).or.(bigT.gt.Error))then
        ...
        go to 555
            ...
            if (mod(itrn,50) .eq. 0) then
         write(*,1000) itrn, bigp, bigu, bigv
      endif
...
999   continue
      ...
      do j = 1, ny
         do i = 1, nx
            if (solid(i,j)) then
               write(70,*) x(i), y(j), BLANK
               write(80,*) x(i), y(j), BLANK
               write(90,*) x(i), y(j), BLANK
               write(100,*) x(i), y(j), BLANK
            else
...
1000  format('Iter=',I6,'  bigp=',1PE12.4,
     $       '  bigu=',1PE12.4,'  bigv=',1PE12.4)
