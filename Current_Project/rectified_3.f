...
C================= ZONE 2: AFTER STEP ==================
      write(24,*) 'zone i=',nx-i_step,', j=',j_step,', f=point'
...
!**********************************************************************!
      ...
      do i=1,nx
        do j=1,ny
          solid(i,j) = .false.
          if (x(i).gt.x_step .and. y(j).gt.(LY - h_step)) then
            solid(i,j) = .true.
          end if
        end do
      end do
...
999   continue
      ...
      open(70,file='u.dat')
      write(70,*)'variables = "x", "y", "u", "blank"'
      write(70,*)'zone i=',ny,', j=',nx,', f=point'

      open(80,file='v.dat')
      write(80,*)'variables = "x", "y", "v", "blank"'
      write(80,*)'zone i=',ny,', j=',nx,', f=point'

      open(90,file='p.dat')
      write(90,*)'variables = "x", "y", "p", "blank"'
      write(90,*)'zone i=',ny,', j=',nx,', f=point'

      open(100,file='T.dat')
      write(100,*)'variables = "x", "y", "T", "blank"'
      ...
      do i = 1, nx
        do j = 1, ny
          ...
          if (solid(i,j)) then
            write(70,*) x(i), y(j), u(i,j), 0.0
            write(80,*) x(i), y(j), v(i,j), 0.0
            write(90,*) x(i), y(j), vp(i,j), 0.0
            write(100,*) x(i), y(j), T(i,j), 0.0
          else
            write(70,*) x(i), y(j), u(i,j), 1.0
            write(80,*) x(i), y(j), v(i,j), 1.0
            write(90,*) x(i), y(j), vp(i,j), 1.0
            write(100,*) x(i), y(j), T(i,j), 1.0
          end if
...
!**********************************************************************!
     ...
     $  +(u(i,j-1)*dy(j)+u(i,j)*dy(j-1))/dyb(j))
      ...
      write(130,*) 'variables = "x", "y", "psi", "blank"'
      write(130,*) 'zone i=',ny,', j=',nx,', f=point'

      do i = 1, nx
         do j = 1, ny
            if (solid(i,j)) then
               write(130,*) x(i), y(j), si(i,j), 0.0
            else
                write(130,*) x(i), y(j), si(i,j), 1.0
            end if
        end do
        write(130,*)
        end do
