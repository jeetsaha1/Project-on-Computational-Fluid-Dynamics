...
C      logical, dimension(:,:), allocatable :: solid
        integer jwall
        integer iterp

      real*8 bigp_old
      real*8 ErrorP, ErrorUVT
      integer maxIter
...
!**********************************************************************!
      nx=201 ! no. of grid in x-direction
      ny=61 ! no. of grid in y-direction
      n=3 ! dimenssion
      LX=5.0
...
C     vertical step
      write(24,*) x_step, LY
      write(24,*) x_step, LY-h_step

C     top wall (left)
      do i=1,nx
        if (x(i).le.x_step) write(24,*) x(i), LY
      end do

C     top wall (right)
      do i=1,nx
        if (x(i).ge.x_step) write(24,*) x(i), LY-h_step
      end do
...
!****************** condition on pressure correction ******************!
      ...
      if (bigp.gt.ErrorP .and.
     $    abs(bigp-bigp_old).gt.1.0E-6) then

        iterp = iterp + 1
        write(*,*) 'P-iter=',iterp,' bigp=',bigp

        bigp_old = bigp

        if (iterp.gt.200) then
          write(*,*) 'WARNING: Pressure stagnated'
          go to 555
        end if
...
C---- check convergence of u, v, T ------------------------
      if (bigu.lt.ErrorUVT .and. bigv.lt.ErrorUVT .and.
     $    bigT.lt.ErrorUVT) then

        write(*,*) 'CONVERGED:',
     $             ' bigu=',bigu,' bigv=',bigv,' bigT=',bigT
        go to 999
      else
